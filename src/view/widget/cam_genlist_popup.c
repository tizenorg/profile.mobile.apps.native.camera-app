...
static Evas_Object *__cam_genlist_popup_gl_icon_get(void *data, Evas_Object *obj, const char *part)
{
	...
	if (!strcmp(part, "elm.swallow.end")) {
		...
		if ((genlist_item->genlist_popup_instance->menu_type) != CAM_MENU_VOLUME_KEY) {
			...
			Eina_Bool retval = elm_image_file_set(icon, edj_path, icon_name);
			cam_debug(LOG_UI, " edj_path = %s", edj_path);
			cam_debug(LOG_UI, "elm_image_file_set retval = %d", retval);
			cam_debug(LOG_UI, "icon_name = %s", icon_name);
			return icon;
		}
	}
